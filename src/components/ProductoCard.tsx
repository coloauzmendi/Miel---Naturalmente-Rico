...
export default function ProductoCard({ producto }: { producto: Producto }) {
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-linea bg-crema-alta transition-shadow hover:shadow-[0_8px_24px_-8px_rgba(43,36,32,0.25)]">
      <Link href={`/productos/${producto.id}`} className="block aspect-[4/3] overflow-hidden bg-linea/60">
        {producto.imagen_url ? (
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <Link href={`/productos/${producto.id}`}>
          <h3 className="font-display text-lg leading-snug text-tinta">{producto.nombre}</h3>
        </Link>
        <p className="line-clamp-2 text-sm text-tinta/70">{producto.descripcion}</p>
